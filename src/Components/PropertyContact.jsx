...
const PropertyContact = ({ propertyId }) => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setData({ ...data, message: "", phone: "", preferredDate: "", preferredTime: "" });
    } catch {
  ...
  return (
    <form onSubmit={submit} className="bg-white shadow rounded p-4">
      <h3 className="text-xl font-semibold mb-3">Contact Agent / Schedule Viewing</h3>
      <div className="grid md:grid-cols-2 gap-3">
        ...
        <textarea className="border p-2 rounded md:col-span-2" rows="3" placeholder="Message"
          value={data.message} onChange={(e)=>setData({...data, message:e.target.value})}/>
      </div>
      <button className="mt-3 bg-green-600 text-white px-4 py-2 rounded">Send</button>
